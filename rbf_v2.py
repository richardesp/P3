...
@click.command()
@click.option('--radii_adjust_type', '-H', default='mean_radii_centroids', help='Type of radii adjustment')
...
def train_rbf_total(radii_adjust_type, train_file, test_file, classification, ratio_rbf, l2, eta, fairness, outputs,
                    model, pred):
    ...
    if not pred:
        ...
        for s in range(1, 6, 1):
            ...
            train_results, test_results = \
                train_rbf(radii_adjust_type, train_file, test_file, classification, ratio_rbf, l2, eta, fairness,
                          outputs,
...
def train_rbf(radii_adjust_type, train_file, test_file, classification, ratio_rbf, l2, eta, fairness, outputs,
              model_file=""):
    ...
    # 3. Adjust radii
    radii = calculate_radii(centers, num_rbf, radii_adjust_type)
...
def calculate_radii(centers, num_rbf, type_adjustment):
    ...
    if type_adjustment == 'mean_radii_centroids':
        center_distances = squareform(pdist(centers, 'euclidean'))
        radii = sum(center_distances) / (2.0 * (num_rbf - 1))
    elif type_adjustment == 'max_distance_between_centers':
        center_distances = squareform(pdist(centers, 'euclidean'))
        radii = np.max(center_distances) / 2.0
    elif type_adjustment == 'mean_distance_between_centers':
        center_distances = squareform(pdist(centers, 'euclidean'))
        radii = np.mean(center_distances) / 2.0
    elif type_adjustment == 'min_max_distance_between_centers':
        center_distances = squareform(pdist(centers, 'euclidean'))
        radii = np.max(center_distances) - np.min(center_distances) / 2.0
    else:
        raise ValueError('Invalid type_adjustment value')
    return radii
